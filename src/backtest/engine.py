...
def run_simulation(data_path=DATA_FILE, holding_days=10):
    print(f"🚀 Starting Backtest Simulation (Max Hold: {holding_days} days)...")
    ...
    for symbol in symbols:
        ...
        for i in range(WARM_UP_DAYS, len(stock_df) - 1):
            ...
            if symbol in active_positions:
                ...
                # HATA DÜZELTME: 'tomorrow' verisine bakarak stop çekemeyiz (Intraday hile olur).
                # 'today' (dün kapanışı) verisine göre stopu güncelleyip 'tomorrow' için hazırlarız.
                
                # Mantık: Fiyat, Giriş + (1 * ATR) kadar yükseldi mi?
                ...
                if today['High'] >= breakeven_trigger:
                    # Yeni Stop: Giriş Fiyatı + Komisyon Payı (0.1 ATR)
                    new_stop = trade['entry_price'] + (0.1 * atr_entry)
                    
                    # Stop'u SADECE YUKARI taşı
                    if new_stop > trade['stop_price']:
                        trade['stop_price'] = new_stop
                        # trade['note'] = 'Trailing Active' # Debug için açılabilir

                # --- EXIT CHECKS (Tomorrow's Price Action) ---
                ...
                if tomorrow['Low'] <= trade['stop_price']:
                    ...
                    # Slippage/Gap Down Logic:
                    # Eğer açılış stopun altındaysa, açılıştan çıkarız.
            ...
            # Mevcut kodunun Data Mapping kısmı aynen kalıyor...
            row_data = {
            ...
            signal, score, stop_price, target_price = trading_engine.evaluate_stock(row_data)
            ...
            if (signal == 'BUY' or signal == 'STRONG_BUY'):
                active_positions[symbol] = {
                    ...
                    'atr_at_entry': row_data['atr'], # Bunu saklamak kritik!
