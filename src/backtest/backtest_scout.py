...
RESULTS_FILE = os.path.join(BASE_DIR, "data", "backtest_results_scout.csv")

# Backtest Settings
INITIAL_CAPITAL = 100000 
WARM_UP_DAYS = 250

def run_simulation(df=None, data_path=DATA_FILE, holding_days=40, min_score=60):
    if df is None:
        print(f"🚀 Starting Scout Strategy Backtest (Max Hold: {holding_days} days, Min Score: {min_score})...")
    ...
    for symbol in symbols:
        ...
        for i in range(WARM_UP_DAYS, len(stock_df) - 1):
            ...
            signal, score, stop_price, target_price = scout_engine.evaluate_stock(row_data)
...
if __name__ == "__main__":
    results = run_simulation(holding_days=40, min_score=60)
