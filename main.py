...
def hisse_islemcisi(sembol):
    ...
    try:
        ...
        if sonuc:
            # Tuple'ı aç (unpack)
            fiyat, sma50, sma200, fk, pd_dd, rsi, macd_line, macd_signal, macd_hist, adx, dmp, dmn, hacim_orani, buyume_orani= sonuc
            
            # Veritabanına yaz
            database.veriyi_kaydet(sembol, fiyat, sma50, sma200, fk, pd_dd, rsi, macd_line, macd_signal, macd_hist, adx, dmp, dmn, hacim_orani, buyume_orani)
        else:
